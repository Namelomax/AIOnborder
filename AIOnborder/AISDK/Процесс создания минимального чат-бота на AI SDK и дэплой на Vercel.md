# AI SDK
Фреймворк использует TypeScript  как основной язык разработки. Разделяется на AI SDK Core который отвечает за работу с LLM и AI SDK UI который отвечает за визуал, например useChat который я использовал для отображения сообщения в реальном времени.
...
При запуске если будет не хватать какой-то зависимости её легко также установить через npm install
## 5. Дэплой на Vercel 
